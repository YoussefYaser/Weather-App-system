...
function showData(){
    ...
    for(var i=0; i<3; i++){
        var myDate = new Date(myCity.location.localtime);
        if(i===0){
            ...
            content[0].children[1].children[0].prepend(myCity.forecast.forecastday[0].day["maxtemp_c"]); // max temp
            ...
            header[i].innerHTML = days[myDate.getDay()+i]; //day name
